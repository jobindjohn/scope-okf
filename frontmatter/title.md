...
* The filename is the stable, path-safe identifier; `title` is the readable label — they can diverge (e.g. `spec-okf.md` has `title: spec-okf`, but a file like `frontmatter/tags.md` has `title: Tags`, not `tags`).
...
* `title` is optional, like `category` and `tags` — missing values don't break conformance (per spec-okf.md's conformance rules).
