...
# What `llm-authorship` / `llm-review` / `llm-human-only-lock` are for

These are three flat fields that record an LLM's relationship to a note in one place: how much of the text an LLM produced (`llm-authorship`), whether a human has vetted it (`llm-review`), and whether agents may edit it without sign-off (`llm-human-only-lock`). They replace the earlier standalone `authorship` scalar, and the later nested `llm` map that briefly superseded it — kept flat rather than nested because Obsidian's Properties panel has no "object" type, so a nested map is a permanent type-mismatch source there. Each field now maps onto a native Obsidian property type: `llm-authorship` is a Number, `llm-review` is Text, `llm-human-only-lock` is a Checkbox.

Together they cover the questions that matter once experts and agents co-author a wiki: *who wrote this*, *can I trust it yet*, and *am I allowed to change it*. Keeping them under a shared `llm-` prefix means a reader — human or agent — can still spot the whole provenance-and-trust picture at a glance.
...
When `llm-authorship` is `1`–`4`, the note also carries a detailed provenance record (the prompt, the model): inline for `1`, in a `.llm/` sidecar for `2`–`4`. See [LLM provenance](/llm.md) for that mechanism, including the `1`→`2` migration.

## `llm-review` (unreviewed | reviewed | verified)

The trust signal, deliberately separate from `llm-authorship`. Authorship says who produced the text; `llm-review` says whether a human has vetted it.
...
`llm-review` can be set even when a note has no LLM involvement (`llm-authorship: 5`) — a hand-written note can still be unreviewed, and a stale hand-written fact is no safer than an agent-written one. It sits alongside `llm-authorship` because vetting is most urgent for LLM-produced text, but it is genuinely a content-trust signal for any author.
...
* Every field is optional, like `category` and `tags` — missing `llm-authorship`, `llm-review`, or `llm-human-only-lock` don't break conformance (per scope-okf.md's conformance rules). An out-of-range `llm-authorship` or unknown `llm-review` value is tolerated too.
* They reflect the *current* state of the note, not its full history. Update `llm-authorship` to whatever level best describes the latest substantive pass; move `llm-review` back to `unreviewed` if a note is materially rewritten after being reviewed.
* Set or update these fields at the same time you'd update `timestamp` — when authorship, review status, or the lock materially changes, bump both together.
* Don't let them become a junk drawer. They are for the LLM's involvement and what agents may do here. Content-classification goes in `category`, subject keywords in `tags`, and note-level concerns that apply regardless of author — freshness, sensitivity, superseding — in their own fields.
...
* (For LLMs) Set `llm-review: unreviewed` on notes you produce unless the human has actually vetted them in the same session. Don't self-certify `reviewed` or `verified` — those are the human's to assert.
* (For LLMs) Treat `llm-human-only-lock: true` as binding: propose the change, wait for explicit go-ahead, then write.
* Don't retroactively backfill `llm-authorship`/`llm-review` on old notes unless you're confident how they were produced and vetted.
