...
Notes on the `aliases` field, expanding on its definition in [scope-okf.md](/scope-okf.md) and its role in [Linking](/scope-okf.md#linking).
...
* Aliases are as collision-prone as titles. Two notes sharing an alias are ambiguous and fall to the tie-break defined in [Linking](/scope-okf.md#linking) (nearest note first — same folder, then widening outward). Keep aliases distinctive.
...
* `aliases` is optional and open-ended — missing values don't break conformance (per scope-okf.md's conformance rules), same as `tags`.
...
* (For LLMs) When resolving a `[[name]]`, follow the resolver order in [Linking](/scope-okf.md#linking) exactly — `id` → basename → `title`/`aliases` — and treat an ambiguous or unresolvable name as tolerated, not an error.
